...
function handleClick(element) {
    const cardNode = element.parentNode;
    const card = {
...
function showMoreText(card) {
    ...
    card.description = `
    <div class = "desc">Описание товара, характеристики товара, инструкция, цена
    </div>
    `;
    card.productName.insertAdjacentHTML('afterend', card.description);
...
function handleClick(clickedButtonEvent) {
 ...
 const card = {
    ...
    if () { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        ...
        showMoreText(card);
    } else if () { // 4.3 проверяем равняется ли текст на кнопке строке 
    "Отмена"
